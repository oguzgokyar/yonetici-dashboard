...
export async function POST(request: Request) {
  const input = await request.json() as { projectId?: string; prompt?: string; contentType?: string; selectedFields?: string[]; idea?: { title?: string; concept?: string; visualDirection?: string } };
  if (!input.projectId || !input.prompt?.trim()) return Response.json({ ok: false, message: "Prompt gerekli." }, { status: 400 });
  const project = getDatabase().prepare("SELECT brand_json, brand_concept_json FROM projects WHERE id = ?").get(input.projectId) as ProjectRow | undefined;
  ...
  try {
    const { content, model } = await completeText("Sen sosyal medya reklam görselleri için uzman prompt yazarı ve art direktörsün. Yanıtın tamamını Türkçe yaz. Sadece geliştirilmiş promptu döndür; açıklama, başlık veya tırnak kullanma.", `Temel prompt: ${input.prompt.trim()}${idea}\nİçerik tipi: ${input.contentType || "Otomatik"}\nMarka: ${brand.brandName || ""}\nSektör: ${brand.industry || ""}\nHedef kitle: ${brand.audience || ""}\nTon: ${brand.tone || ""}\n${brandConceptInstruction(brandConcept)}\nUygulamanın sonradan ekleyeceği doğrulanmış marka kaynakları: ${selected || "Yok"}\n\nPromptu Türkçe olarak sahne, kompozisyon, ışık, kamera, materyal, duygu ve reklam odağı bakımından geliştir. Marka konseptini zorunlu tasarım sınırı olarak uygula. Görsel modeline hiçbir metin, harf, sayı, logo, renk kodu, filigran veya tabela üretmemesini; marka öğeleri için doğal negatif alan bırakmasını açıkça söyle. Seçilmemiş iletişim bilgilerini ekleme ve marka bilgisi uydurma.`, { temperature: 0.45, maxTokens: 900 });
