...
export async function POST(request: Request) {
  const input = await request.json() as { projectId?: string; prompt?: string };
  ...
  const project = database.prepare("SELECT brand_json FROM projects WHERE id = ?").get(projectId) as ProjectRow | undefined;
  ...
  try {
    const instruction = `Ürün/içerik bilgisi: ${sourcePrompt}\nMarka: ${brand.brandName || "Belirtilmedi"}\nSektör: ${brand.industry || "Belirtilmedi"}\nHedef kitle: ${brand.audience || "Belirtilmedi"}\nMarka tonu: ${brand.tone || "Belirtilmedi"}\n\nDaha önce önerilen veya kullanılan fikirler (bunları ve yakın varyasyonlarını tekrar etme):\n${excluded}\n\nBirbirinden belirgin biçimde farklı 7 reklam görseli fikri üret. Her fikir uygulanabilir, tek karede anlatılabilir ve görsel üretim modeline açık yön verebilmelidir. Türkçe yaz. Yalnızca JSON dizi döndür. Her nesne: {"title":"kısa fikir adı","concept":"2 cümlelik pazarlama fikri","visualDirection":"kompozisyon, sahne, ışık ve odak","suggestedPrompt":"doğrudan görsel üretiminde kullanılabilecek ayrıntılı prompt"}.`;
